...
iotc_bsp_io_net_socket_connect(iotc_bsp_socket_t* iotc_socket, const char* host,
  ...
  struct zsock_addrinfo *result, *rp = NULL;
  int status;
  ...
  // Address resolution.
  status = zsock_getaddrinfo(host, NULL, &hints, &result);
  if (0 != status) {
    return IOTC_BSP_IO_NET_STATE_ERROR;
  }

  for (rp = result; rp != NULL; rp = rp->ai_next) {
    *iotc_socket = zsock_socket(rp->ai_family, rp->ai_socktype, rp->ai_protocol);
    if (-1 == *iotc_socket)
      continue;

    // Set the socket to be non-blocking
    const int flags = zsock_fcntl(*iotc_socket, F_GETFL, 0);
    if (-1 == zsock_fcntl(*iotc_socket, F_SETFL, flags | O_NONBLOCK)) {
    ...
    // Attempt to connect.
    status = zsock_connect(*iotc_socket, rp->ai_addr, rp->ai_addrlen);
...
iotc_bsp_io_net_state_t iotc_bsp_io_net_write(iotc_bsp_socket_t iotc_socket,
  ...
  int errval = 0;
  *out_written_count = zsock_send(iotc_socket, buf, count, 0);
...
iotc_bsp_io_net_state_t iotc_bsp_io_net_read(iotc_bsp_socket_t iotc_socket,
  ...
  int errval = 0;
  *out_read_count = zsock_recv(iotc_socket, buf, count, ZSOCK_MSG_DONTWAIT);
...
#define _FD_SET(socket, event, pollfd)                                          \
  pollfd.fd = socket;                                                          \
  pollfd.events |= event;
#define _FD_ISSET(event, pollfd) (pollfd.revents | event)

iotc_bsp_io_net_state_t
iotc_bsp_io_net_select(iotc_bsp_socket_events_t* socket_events_array,
                       size_t socket_events_array_size, long timeout_sec) {
  struct zsock_pollfd fds[1]; // note: single socket support
  ...
  for (socket_id = 0; socket_id < socket_events_array_size; ++socket_id) {
    ...
    if (1 == socket_events->in_socket_want_read) {
      _FD_SET(socket_events->iotc_socket, ZSOCK_POLLIN, fds[0]);
    }

    if ((1 == socket_events->in_socket_want_write) ||
        (1 == socket_events->in_socket_want_connect)) {
      _FD_SET(socket_events->iotc_socket, ZSOCK_POLLOUT, fds[0]);
    }

    if (1 == socket_events->in_socket_want_error) {
      _FD_SET(socket_events->iotc_socket, ZSOCK_POLLERR, fds[0]);
    }
  }

  /* call the actual posix select */
  const int result = zsock_poll(fds, 1, timeout_sec);
  ...
  if (0 < result) {
    ...
    for (socket_id = 0; socket_id < socket_events_array_size; ++socket_id) {
      ...
      if (_FD_ISSET(ZSOCK_POLLIN, fds[0])) {
        socket_events->out_socket_can_read = 1;
      }

      if (_FD_ISSET(ZSOCK_POLLOUT, fds[0])) {
      ...
      if (_FD_ISSET(ZSOCK_POLLERR, fds[0])) {
